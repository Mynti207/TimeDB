...
def request_augmented_select(proc, target, arg=None, md={}, additional=None):
    ...
    r = requests.get("http://127.0.0.1:8080/tsdb/augmented_select",
                     data=json.dumps(msg))
    print('r.text: ', r.text)
    return json.loads(r.text, object_pairs_hook=OrderedDict)
...
def request_add_trigger(proc, onwhat, target, arg=None):
    ...
    r = requests.post("http://127.0.0.1:8080/tsdb/add_trigger",
                      data=json.dumps(msg))


def request_remove_trigger(proc, onwhat):
    msg = TSDBOp_RemoveTrigger(proc, onwhat).to_json()
    r = requests.post("http://127.0.0.1:8080/tsdb/remove_trigger",
                      data=json.dumps(msg))


def request_similarity_search(query, top=1):
    msg = {'query': query.to_json(), 'top': top}
    r = requests.get("http://127.0.0.1:8080/tsdb/similarity_search",
                     data=json.dumps(msg))
...
def main():
    ...
    for i, m, s, j in zip(range(50), mus, sigs, jits):
        ...
        pk = "ts-{}".format(i)
        tsdict[pk] = tsrs
        meta['vp'] = False  # augment metadata w/ boolean asking if is a vp
        metadict[pk] = meta

    # choose 5 distinct vantage point time series
    vpkeys = ["ts-{}".format(i) for i in
              np.random.choice(range(50), size=5, replace=False)]
    for i in range(5):
        # add 5 triggers to upsert distances to these vantage points
        request_add_trigger('corr', 'insert_ts', ["d_vp-{}".format(i)],
                            tsdict[vpkeys[i]])
    ...
    print('------------order >= 4  order, blarg and mean sent back,'
          'also sorted---------')
    results = request_select({'order': {'>=': 4}},
    ...
    results = request_select({'blarg': {'>=': 1}, 'order': 1},
                             fields=['blarg', 'std'])
    for k in results:
        print(k, results[k])
    ...
    vpdist = {}
    result_distance = request_augmented_select('corr',
                                               ['vpdist'],
                                               query,
                                               {'vp': {'==': True}})
